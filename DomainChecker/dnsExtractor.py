...
def clean_dmarc(record):
    ...
    for part in record.split(";"):
        ...
        part_lower = part.lower().strip()  # Pour éviter de répéter cette opération
        if part_lower.startswith("pct=") or part_lower.startswith("sp=") or part_lower.startswith("aspf="):
            continue
...
if __name__ == "__main__":
    ...
    domain = domainChecker.validate_domain(input_value)
    print(domain)
    diffs = compare_records(domain.strip())
